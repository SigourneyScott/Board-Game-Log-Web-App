...
router.get("/:_id", (req, res) => {
  ...
  import_session_svc.default.get(_id).then((session) => res.json(session)).catch((err) => res.status(404).send(err));
});
router.post("/", (req, res) => {
...
router.put("/:_id", (req, res) => {
  const { _id } = req.params;
  const newSession = req.body;
  import_session_svc.default.update(_id, newSession).then((session) => res.json(session)).catch((err) => res.status(404).end());
});
router.delete("/:_id", (req, res) => {
  const { _id } = req.params;
  import_session_svc.default.remove(_id).then(() => res.status(204).end()).catch((err) => res.status(404).send(err));
});
